...
(function () {
  ...
  const OPS = [
    ...
    { id: "crockford32-encode", name: "To Crockford Base32", cat: "Encoding", run: (s) => CL.crockford32Encode(CL.utf8Bytes(s)), about: "Encodes text as one big number in Crockford's Base32 - digits and letters, with I, L, O and U dropped to avoid confusion with 1, 1, 0 and V. Popular for IDs a person might type by hand.", example: { in: "Hi", out: "J39" } },
    { id: "crockford32-decode", name: "From Crockford Base32", cat: "Encoding", run: (s) => CL.bytesToText(CL.crockford32Decode(s)), about: "Decodes Crockford Base32 back into text." },
    { id: "z85-encode", name: "To Z85", cat: "Encoding", run: (s) => CL.z85Encode(CL.utf8Bytes(s)), about: "ZeroMQ's Base85 variant (RFC 32) - denser than Base64, 4 bytes become 5 characters, using an 85-character alphabet without Ascii85's quote/backslash-heavy symbol set.", example: { in: "Byte", out: "lv0V:" } },
    { id: "z85-decode", name: "From Z85", cat: "Encoding", run: (s) => CL.bytesToText(CL.z85Decode(s)), about: "Decodes Z85 back into text." },
    { id: "bcd-encode", name: "To BCD", cat: "Encoding", run: (s) => CL.bcdEncode(s), about: "Binary-Coded Decimal: each decimal digit becomes its own 4-bit nibble, two digits packed per byte, shown as hex. An odd number of digits is padded with a trailing 0.", example: { in: "123", out: "1230" } },
    { id: "bcd-decode", name: "From BCD", cat: "Encoding", run: (s) => CL.bcdDecode(s), about: "Decodes BCD hex back into its decimal digits.", example: { in: "1230", out: "1230" } },
    { id: "mime-word-encode", name: "To MIME Encoded-Word", cat: "Encoding", run: (s) => CL.mimeWordEncode(s), about: "Wraps text in the RFC 2047 encoded-word format (=?charset?B?...?=) email headers use to carry non-ASCII subject lines and names through 7-bit-only infrastructure.", example: { in: "hi", out: "=?UTF-8?B?aGk=?=" } },
    { id: "mime-word-decode", name: "From MIME Encoded-Word", cat: "Encoding", run: (s) => CL.mimeWordDecode(s), about: "Unwraps an RFC 2047 encoded-word back into plain text.", example: { in: "=?UTF-8?B?aGk=?=", out: "hi" } },
    { id: "utf7-encode", name: "To UTF-7", cat: "Encoding", run: (s) => CL.utf7Encode(s), about: "RFC 2152's 7-bit-safe Unicode encoding, from the days before UTF-8 won - safe ASCII passes through unchanged, a literal + becomes +-, and everything else is shifted into a Base64-encoded run wrapped in +...-.", example: { in: "café", out: "caf+AOk-" } },
    { id: "utf7-decode", name: "From UTF-7", cat: "Encoding", run: (s) => CL.utf7Decode(s), about: "Decodes UTF-7 back into text.", example: { in: "caf+AOk-", out: "café" } },
    { id: "bijective26-encode", name: "To Bijective Base26 (Spreadsheet Columns)", cat: "Encoding", run: (s) => CL.toBijective26(s), about: "Converts a number to spreadsheet-style column letters (1=A, 26=Z, 27=AA, 28=AB...) - a positional system with no zero digit, which is why it skips straight from Z to AA instead of using a two-digit '00'.", example: { in: "27", out: "AA" } },
    { id: "bijective26-decode", name: "From Bijective Base26 (Spreadsheet Columns)", cat: "Encoding", run: (s) => CL.fromBijective26(s), about: "Converts spreadsheet-style column letters back to a number.", example: { in: "AA", out: "27" } },
    { id: "unary-encode", name: "To Unary", cat: "Encoding", run: (s) => CL.unaryEncode(s), about: "The simplest possible number system: writes a number as that many 1s in a row, with no place value at all. Capped at 1000 so the output doesn't get out of hand.", example: { in: "5", out: "11111" } },
    { id: "unary-decode", name: "From Unary", cat: "Encoding", run: (s) => CL.unaryDecode(s), about: "Counts the 1s in a unary number and returns the count.", example: { in: "11111", out: "5" } },
    ...
    { id: "trithemius-encode", name: "Trithemius Cipher Encode", cat: "Ciphers", run: (s) => CL.trithemiusEncode(s), about: "A Caesar cipher whose shift grows by 1 with every letter's position (A+0, B+1, C+2...) - no key to share, but the growing shift defeats simple frequency analysis better than a fixed Caesar shift.", example: { in: "ABC", out: "ACE" } },
    { id: "trithemius-decode", name: "Trithemius Cipher Decode", cat: "Ciphers", run: (s) => CL.trithemiusDecode(s), about: "Reverses the Trithemius cipher's growing shift.", example: { in: "ACE", out: "ABC" } },
    { id: "gronsfeld-encode", name: "Gronsfeld Cipher Encode", cat: "Ciphers", params: [{ name: "key", label: "Key (digits)", type: "text", def: "31415" }], run: (s, p) => CL.gronsfeldEncode(s, p.key), about: "Vigenere with a numeric key instead of a letter keyword - each digit 0-9 is a shift amount, repeating like Vigenere's keyword.", example: { in: "HI", out: "KJ" } },
    { id: "gronsfeld-decode", name: "Gronsfeld Cipher Decode", cat: "Ciphers", params: [{ name: "key", label: "Key (digits)", type: "text", def: "31415" }], run: (s, p) => CL.gronsfeldDecode(s, p.key), about: "Reverses a Gronsfeld cipher with the same numeric key.", example: { in: "KJ", out: "HI" } },
    { id: "autokey-encode", name: "Autokey Cipher Encode", cat: "Ciphers", params: [{ name: "key", label: "Keyword", type: "text", def: "KEY" }], run: (s, p) => CL.autokeyEncode(s, p.key), about: "A Vigenere variant where the key is extended by the plaintext itself instead of repeating the keyword - so the key never repeats short, closing Vigenere's main weakness.", example: { in: "HELLO", out: "RIJSS" } },
    { id: "autokey-decode", name: "Autokey Cipher Decode", cat: "Ciphers", params: [{ name: "key", label: "Keyword", type: "text", def: "KEY" }], run: (s, p) => CL.autokeyDecode(s, p.key), about: "Reverses an Autokey cipher with the same starting keyword, reconstructing the extended key as it decodes.", example: { in: "RIJSS", out: "HELLO" } },
    { id: "nihilist-encode", name: "Nihilist Cipher Encode", cat: "Ciphers", params: [{ name: "key", label: "Keyword", type: "text", def: "KEY" }], run: (s, p) => CL.nihilistEncode(s, p.key), about: "Plots each letter on the Polybius square, then adds a repeating numeric key (the keyword run through the same grid) to each coordinate pair - output is a run of two/three-digit numbers.", example: { in: "HI", out: "48 39" } },
    { id: "nihilist-decode", name: "Nihilist Cipher Decode", cat: "Ciphers", params: [{ name: "key", label: "Keyword", type: "text", def: "KEY" }], run: (s, p) => CL.nihilistDecode(s, p.key), about: "Subtracts the same numeric key back out of each number, then reads the result as Polybius coordinates.", example: { in: "48 39", out: "HI" } },
    { id: "keyword-cipher-encode", name: "Keyword Cipher Encode", cat: "Ciphers", params: [{ name: "key", label: "Keyword", type: "text", def: "ZEBRA" }], run: (s, p) => CL.keywordCipherEncode(s, p.key), about: "A monoalphabetic substitution cipher: write the keyword's unique letters first, then the rest of the alphabet in order, and substitute plaintext letters by their position in that scrambled alphabet.", example: { in: "CAB", out: "BZE" } },
    { id: "keyword-cipher-decode", name: "Keyword Cipher Decode", cat: "Ciphers", params: [{ name: "key", label: "Keyword", type: "text", def: "ZEBRA" }], run: (s, p) => CL.keywordCipherDecode(s, p.key), about: "Reverses a Keyword cipher with the same keyword.", example: { in: "BZE", out: "CAB" } },
    { id: "rot18", name: "ROT18", cat: "Ciphers", run: (s) => CL.rot18(s), about: "ROT13 on letters plus ROT5 on digits, applied in one pass. Reciprocal like ROT13 and ROT47 - running it twice restores the original, including any digits.", example: { in: "A1", out: "N6" } },
    { id: "playfair-encode", name: "Playfair Cipher Encode", cat: "Ciphers", params: [{ name: "key", label: "Keyword", type: "text", def: "PLAYFAIR" }], run: (s, p) => CL.playfairEncode(s, p.key), about: "Encrypts pairs of letters (digraphs) using a keyed 5x5 grid (I/J share a cell) instead of single letters, resisting simple frequency analysis far better than any single-letter substitution. Doubled letters and an odd final letter are padded with X.", example: { in: "HIDE", out: "EBIM" } },
    { id: "playfair-decode", name: "Playfair Cipher Decode", cat: "Ciphers", params: [{ name: "key", label: "Keyword", type: "text", def: "PLAYFAIR" }], run: (s, p) => CL.playfairDecode(s, p.key), about: "Reverses a Playfair cipher with the same keyword. If padding letters were inserted during encoding, they'll appear in the decoded output too.", example: { in: "EBIM", out: "HIDE" } },
    ...
    { id: "nand", name: "NAND", cat: "Bitwise", params: [{ name: "key", label: "Key", type: "text", def: "key" }], run: (s, p) => CL.bitNandHex(s, p.key), about: "Bitwise NAND with a repeating key, output as hex. A bit is 0 only if both inputs are 1 - the inverse of AND." },
    { id: "nor", name: "NOR", cat: "Bitwise", params: [{ name: "key", label: "Key", type: "text", def: "key" }], run: (s, p) => CL.bitNorHex(s, p.key), about: "Bitwise NOR with a repeating key, output as hex. A bit is 1 only if both inputs are 0 - the inverse of OR." },
    { id: "xnor", name: "XNOR", cat: "Bitwise", params: [{ name: "key", label: "Key", type: "text", def: "key" }], run: (s, p) => CL.bitXnorHex(s, p.key), about: "Bitwise XNOR with a repeating key, output as hex. A bit is 1 when both inputs match - the inverse of XOR." },
    { id: "arith-shift-right", name: "Arithmetic Shift Right", cat: "Bitwise", params: [{ name: "amount", label: "Amount", type: "number", def: 1 }], run: (s, p) => CL.arithShiftRightHex(s, p.amount), about: "Shifts each byte's bits right, preserving the sign bit (unlike a logical shift, which always fills with 0). Matters for signed-number division by powers of two.", example: { in: "é", out: "e1d4" } },
    { id: "parity-bit", name: "Parity Bit", cat: "Bitwise", run: (s) => CL.parityBit(s), about: "Counts the total 1-bits across the whole input and reports even or odd parity - the simplest error-detection scheme, used to catch a single flipped bit in transit.", example: { in: "A", out: "0 (even)" } },
    { id: "to-twos-complement", name: "To Two's Complement", cat: "Bitwise", params: [{ name: "bits", label: "Bit width", type: "number", def: 8 }], run: (s, p) => CL.toTwosComplement(s, p.bits), about: "Converts a signed decimal number into its two's complement hex representation at a chosen bit width - how negative numbers are actually stored in memory.", example: { in: "-1", out: "ff" } },
    { id: "from-twos-complement", name: "From Two's Complement", cat: "Bitwise", params: [{ name: "bits", label: "Bit width", type: "number", def: 8 }], run: (s, p) => CL.fromTwosComplement(s, p.bits), about: "Interprets hex as a two's complement signed integer at a chosen bit width and outputs the decimal value.", example: { in: "ff", out: "-1" } },
    { id: "toggle-bit", name: "Toggle Bit", cat: "Bitwise", params: [{ name: "position", label: "Bit position (from LSB)", type: "number", def: 0 }], run: (s, p) => CL.toggleBitHex(s, p.position), about: "Flips a single bit at a chosen position (0 = least significant bit) in a hex value and returns the result.", example: { in: "00", out: "01" } },
    { id: "count-leading-zeros", name: "Count Leading Zeros", cat: "Bitwise", params: [{ name: "bits", label: "Bit width", type: "number", def: 8 }], run: (s, p) => CL.countLeadingZerosHex(s, p.bits), about: "Counts how many leading 0 bits a hex value has at a chosen bit width - a primitive used inside fast integer-log2 and normalization routines.", example: { in: "01", out: "7" } },
    { id: "bit-length", name: "Bit Length", cat: "Bitwise", run: (s) => CL.bitLength(s), about: "Reports how many bits are needed to represent a non-negative decimal number, with no leading zeros.", example: { in: "255", out: "8" } },
    ...
    { id: "crc8", name: "CRC-8", cat: "Hashing", run: (s) => CL.crc8(CL.utf8Bytes(s)), about: "CRC-8/SMBUS - an 8-bit checksum, smaller and faster than CRC-16/CRC-32, used inside SMBus and small embedded protocols. Catches accidental corruption, not tampering.", example: { in: "123456789", out: "f4" } },
    { id: "fletcher16", name: "Fletcher-16", cat: "Hashing", run: (s) => CL.fletcher16(CL.utf8Bytes(s)), about: "Two running byte sums mod 255, packed into 16 bits - simpler and faster than Adler-32, at the cost of being weaker at catching certain error patterns.", example: { in: "abcde", out: "c8f0" } },
    { id: "fletcher32", name: "Fletcher-32", cat: "Hashing", run: (s) => CL.fletcher32(CL.utf8Bytes(s)), about: "Fletcher's checksum computed over 16-bit words instead of bytes, for a stronger 32-bit result.", example: { in: "abcde", out: "4ff029c7" } },
    { id: "fnv1a32", name: "FNV-1a (32-bit)", cat: "Hashing", run: (s) => CL.fnv1a32(CL.utf8Bytes(s)), about: "A fast, simple non-cryptographic hash (multiply by a prime, XOR in each byte) used inside hash tables and bloom filters - not for security.", example: { in: "foobar", out: "bf9cf968" } },
    { id: "fnv1a64", name: "FNV-1a (64-bit)", cat: "Hashing", run: (s) => CL.fnv1a64(CL.utf8Bytes(s)), about: "The 64-bit variant of FNV-1a, for a much lower collision rate than the 32-bit version at the same non-cryptographic speed.", example: { in: "a", out: "af63dc4c8601ec8c" } },
    { id: "djb2", name: "DJB2", cat: "Hashing", run: (s) => CL.djb2(CL.utf8Bytes(s)), about: "Daniel Bernstein's classic string hash (hash * 33 + byte) - simple enough to implement from memory, still used in some hash table implementations.", example: { in: "hello", out: "0f923099" } },
    { id: "sdbm", name: "SDBM", cat: "Hashing", run: (s) => CL.sdbm(CL.utf8Bytes(s)), about: "Another simple multiplicative string hash, originally from the sdbm database library - a common alternative to DJB2 with different collision behavior.", example: { in: "hello", out: "28d19932" } },
    { id: "checksum8", name: "Sum of Bytes (mod 256)", cat: "Hashing", run: (s) => CL.checksum8(CL.utf8Bytes(s)), about: "The simplest possible checksum - add every byte together and wrap at 256. Catches almost nothing on its own, but is the building block inside Luhn-style and protocol checksums.", example: { in: "AA", out: "82" } },
    { id: "hmac-md5", name: "HMAC-MD5", cat: "Hashing", params: [{ name: "key", label: "Key", type: "text", def: "key" }], run: (s, p) => CL.hmacMd5Hex(p.key, s), about: "Keyed hash using MD5, built from scratch (MD5 isn't in the SHA-only Web Crypto HMAC API). MD5 itself is broken for security, so prefer HMAC-SHA256 for anything that matters.", example: { in: "The quick brown fox jumps over the lazy dog", out: "80070713463e7749b90c2dc24911e275" } },
    { id: "internet-checksum", name: "Internet Checksum (RFC 1071)", cat: "Hashing", run: (s) => CL.internetChecksum(CL.utf8Bytes(s)), about: "The one's-complement checksum inside every IPv4, TCP and UDP header - sum all 16-bit words with end-around carry, then complement the result. Catches accidental corruption, not tampering.", example: { in: "hello", out: "bc2d" } },
    { id: "xor8-checksum", name: "XOR-8 Checksum", cat: "Hashing", run: (s) => CL.xor8Checksum(CL.utf8Bytes(s)), about: "XORs every byte together into a single check byte - the simplest checksum that can still catch a single-bit error, though two errors in the same bit position cancel out undetected.", example: { in: "AA", out: "00" } },
    ...
    { id: "remove-line-numbers", name: "Remove line numbers", cat: "Text", run: (s) => CL.removeLineNumbers(s), about: "Strips a leading number (and an optional separator like . or :) from each line - the reverse of Add line numbers.", example: { in: "1  a\n2  b", out: "a\nb" } },
    { id: "tabs-to-spaces", name: "Tabs to spaces", cat: "Text", params: [{ name: "width", label: "Spaces per tab", type: "number", def: 4 }], run: (s, p) => CL.tabsToSpaces(s, p.width), about: "Replaces every tab character with a fixed number of spaces." },
    { id: "spaces-to-tabs", name: "Spaces to tabs", cat: "Text", params: [{ name: "width", label: "Spaces per tab", type: "number", def: 4 }], run: (s, p) => CL.spacesToTabs(s, p.width), about: "Replaces runs of spaces of a given width with a tab character." },
    { id: "remove-duplicate-words", name: "Remove duplicate words", cat: "Text", run: (s) => CL.removeDuplicateWords(s), about: "Drops repeated words (case-insensitive), keeping the first occurrence and the surrounding spacing - unlike Unique lines, this works within a single line.", example: { in: "the cat and the dog", out: "the cat and dog" } },
    { id: "pig-latin", name: "Pig Latin", cat: "Text", run: (s) => CL.pigLatinEncode(s), about: "The classic wordplay cipher: words starting with a vowel get 'way' appended; words starting with consonants move those consonants to the end and add 'ay'.", example: { in: "pig latin", out: "igpay atinlay" } },
    { id: "leet-encode", name: "To Leetspeak", cat: "Text", run: (s) => CL.leetEncode(s), about: "Substitutes look-alike digits for letters (a→4, e→3, i→1, o→0, s→5...) - internet slang from 1990s hacker/gaming culture.", example: { in: "leet", out: "1337" } },
    { id: "leet-decode", name: "From Leetspeak", cat: "Text", run: (s) => CL.leetDecode(s), about: "Best-effort reversal of Leetspeak - maps digits back to letters, though the substitution isn't perfectly one-to-one (1 could have been 'i' or 'l').", example: { in: "1337", out: "ieet" } },
    { id: "pad-left", name: "Pad left", cat: "Text", params: [{ name: "width", label: "Width", type: "number", def: 8 }, { name: "ch", label: "Pad character", type: "text", def: "0" }], run: (s, p) => CL.padLeft(s, p.width, p.ch), about: "Pads the start of the text with a repeated character until it reaches a target width.", example: { in: "7", out: "00000007" } },
    { id: "pad-right", name: "Pad right", cat: "Text", params: [{ name: "width", label: "Width", type: "number", def: 8 }, { name: "ch", label: "Pad character", type: "text", def: "0" }], run: (s, p) => CL.padRight(s, p.width, p.ch), about: "Pads the end of the text with a repeated character until it reaches a target width.", example: { in: "7", out: "70000000" } },
    { id: "center-text", name: "Center text", cat: "Text", params: [{ name: "width", label: "Width", type: "number", def: 11 }, { name: "ch", label: "Pad character", type: "text", def: " " }], run: (s, p) => CL.centerText(s, p.width, p.ch), about: "Pads both sides of the text to center it within a target width - any odd leftover space goes on the right." },
    { id: "repeat-text", name: "Repeat text", cat: "Text", params: [{ name: "times", label: "Times", type: "number", def: 3 }], run: (s, p) => CL.repeatText(s, p.times), about: "Repeats the input a given number of times, back to back.", example: { in: "ab", out: "ababab" } },
    { id: "truncate-text", name: "Truncate text", cat: "Text", params: [{ name: "length", label: "Max length", type: "number", def: 20 }], run: (s, p) => CL.truncateText(s, p.length), about: "Cuts text down to a maximum length, appending ... if anything was actually removed." },
    { id: "wrap-in-quotes", name: "Wrap in quotes", cat: "Text", params: [{ name: "quote", label: "Quote character", type: "text", def: "\"" }], run: (s, p) => CL.wrapInQuotes(s, p.quote), about: "Adds a quote character to both ends of the text.", example: { in: "hi", out: "\"hi\"" } },
    { id: "strip-surrounding-quotes", name: "Strip surrounding quotes", cat: "Text", run: (s) => CL.stripSurroundingQuotes(s), about: "Removes a single matching pair of quote characters from the start and end of the text, if present.", example: { in: "\"hi\"", out: "hi" } },
    { id: "alternating-case", name: "aLtErNaTiNg CaSe", cat: "Text", run: (s) => CL.alternatingCase(s), about: "Alternates upper and lower case letter by letter, skipping non-letters without breaking the rhythm - aka 'mocking SpongeBob' case.", example: { in: "hello world", out: "hElLo WoRlD" } },
    { id: "remove-numbers", name: "Remove numbers", cat: "Text", run: (s) => CL.removeNumbers(s), about: "Strips every digit 0-9 from the text.", example: { in: "Room 42", out: "Room " } },
    { id: "remove-punctuation", name: "Remove punctuation", cat: "Text", run: (s) => CL.removePunctuation(s), about: "Strips standard ASCII punctuation and symbol characters, leaving letters, digits and whitespace.", example: { in: "Hi, ByteLabs!", out: "Hi ByteLabs" } },
    { id: "insert-at-position", name: "Insert at position", cat: "Text", params: [{ name: "position", label: "Position", type: "number", def: 0 }, { name: "text", label: "Text to insert", type: "text", def: "" }], run: (s, p) => CL.insertAtPosition(s, p.position, p.text), about: "Inserts a string at a specific character index, shifting the rest of the text right." },
    ...
    { id: "expand-ipv6", name: "Expand IPv6", cat: "Data", run: (s) => CL.expandIpv6(s), about: "Expands a shortened IPv6 address (using ::) into its full 8-group form with every group padded to 4 hex digits.", example: { in: "2001:db8::1", out: "2001:0db8:0000:0000:0000:0000:0000:0001" } },
    { id: "compress-ipv6", name: "Compress IPv6", cat: "Data", run: (s) => CL.compressIpv6(s), about: "Shortens an IPv6 address by dropping leading zeros in each group and collapsing the longest run of all-zero groups into a single ::.", example: { in: "2001:0db8:0000:0000:0000:0000:0000:0001", out: "2001:db8::1" } },
    { id: "format-mac", name: "Format MAC Address", cat: "Data", params: [{ name: "style", label: "Style", type: "select", def: "colon", options: [{ v: "colon", t: "00:11:22:33:44:55" }, { v: "dash", t: "00-11-22-33-44-55" }, { v: "dot", t: "0011.2233.4455 (Cisco)" }, { v: "plain", t: "001122334455" }] }], run: (s, p) => CL.formatMac(s, p.style), about: "Reformats a MAC address (from any of the common separator styles) into the style you choose.", example: { in: "00:11:22:33:44:55", out: "00:11:22:33:44:55" } },
    { id: "extract-hex-colors", name: "Extract Hex Colors", cat: "Data", run: (s) => CL.extractHexColors(s), about: "Pulls every #rrggbb or #rgb hex color code out of a block of text.", example: { in: "background: #ff00aa;", out: "#ff00aa" } },
    { id: "extract-hashtags", name: "Extract Hashtags", cat: "Data", run: (s) => CL.extractHashtags(s), about: "Pulls every #hashtag out of a block of text.", example: { in: "loving #ByteLabs today", out: "#ByteLabs" } },
    { id: "extract-mac", name: "Extract MAC Addresses", cat: "Data", run: (s) => CL.extractMacAddresses(s), about: "Pulls colon- or dash-separated MAC addresses out of a block of text.", example: { in: "host 00:11:22:33:44:55 is up", out: "00:11:22:33:44:55" } },
    { id: "extract-domain", name: "Extract Domain", cat: "Data", run: (s) => CL.extractDomain(s), about: "Pulls the first domain name out of a URL or a plain sentence.", example: { in: "visit https://example.com/page", out: "example.com" } },
    { id: "mask-email", name: "Mask Email Address", cat: "Data", run: (s) => CL.maskEmail(s), about: "Redacts every email address in the text down to its first character plus *** before the @ - handy for sharing logs without leaking full addresses.", example: { in: "contact hello@example.com now", out: "contact h***@example.com now" } },
    { id: "extract-hashes", name: "Extract Hashes", cat: "Data", run: (s) => CL.extractHashes(s), about: "Pulls out anything the right length to be an MD5 (32 hex), SHA-1 (40 hex) or SHA-256 (64 hex) hash." },
    { id: "extract-ipv6", name: "Extract IPv6 Addresses", cat: "Data", run: (s) => CL.extractIpv6(s), about: "Pulls IPv6 addresses out of a block of text, validating each candidate rather than relying on a regex alone (IPv6's :: shorthand is hard to match correctly with regex).", example: { in: "server at 2001:db8::1 responded", out: "2001:db8::1" } },
    { id: "extract-base64-blobs", name: "Extract Base64 Blobs", cat: "Data", run: (s) => CL.extractBase64Blobs(s), about: "Heuristically pulls runs of 16+ Base64-alphabet characters out of a block of text - useful for spotting embedded payloads in logs or malware samples." },
    { id: "csv-to-json", name: "CSV to JSON", cat: "Data", run: (s) => CL.csvToJson(s), about: "Converts CSV (with a header row, quoted fields supported) into a JSON array of objects.", example: { in: "name,age\nAlice,30", out: "[\n  {\n    \"name\": \"Alice\",\n    \"age\": \"30\"\n  }\n]" } },
    { id: "json-to-csv", name: "JSON to CSV", cat: "Data", run: (s) => CL.jsonToCsv(s), about: "Converts a JSON array of flat objects into CSV, using the first object's keys as the header row.", example: { in: "[{\"name\":\"Alice\",\"age\":\"30\"}]", out: "name,age\nAlice,30" } },
    { id: "build-query-string", name: "Build Query String", cat: "Data", run: (s) => CL.buildQueryString(s), about: "Turns key=value lines into a URL query string, percent-encoding each key and value.", example: { in: "item=42\ncolor=blue", out: "?item=42&color=blue" } },
    { id: "unix-sec-to-ms", name: "Unix Seconds to Milliseconds", cat: "Data", run: (s) => CL.unixSecToMs(s), about: "Multiplies a Unix timestamp in seconds by 1000 - useful since JavaScript's Date and many APIs expect milliseconds.", example: { in: "1704067200", out: "1704067200000" } },
    { id: "unix-ms-to-sec", name: "Unix Milliseconds to Seconds", cat: "Data", run: (s) => CL.unixMsToSec(s), about: "Converts a Unix timestamp in milliseconds back to whole seconds.", example: { in: "1704067200000", out: "1704067200" } },
    { id: "date-difference", name: "Date Difference", cat: "Data", params: [{ name: "b", label: "To date (YYYY-MM-DD)", type: "text", def: "2024-01-11" }], run: (s, p) => CL.dateDifference(s, p.b), about: "Counts the whole days between two dates.", example: { in: "2024-01-01", out: "10 days" } },
    { id: "word-frequency", name: "Word Frequency", cat: "Data", run: (s) => CL.wordFrequency(s), about: "Counts how often each word appears (case-insensitive), most frequent first - a word-level companion to Letter frequency.", example: { in: "the cat and the dog", out: "the: 2\nand: 1\ncat: 1\ndog: 1" } },
    ...
    { id: "gen-mac", name: "Random MAC Address", cat: "Random", ignoresInput: true, run: () => CL.randomMac(), about: "A random MAC address with the multicast bit cleared, so it looks like a plausible real device address. Test data only." },
    { id: "gen-hex-color", name: "Random Hex Color", cat: "Random", ignoresInput: true, run: () => CL.randomHexColor(), about: "A random #rrggbb color." },
    { id: "gen-int-range", name: "Random Number in Range", cat: "Random", ignoresInput: true, params: [{ name: "min", label: "Min", type: "number", def: 1 }, { name: "max", label: "Max", type: "number", def: 100 }], run: (s, p) => CL.randomIntInRange(p.min, p.max), about: "A random whole number between min and max, inclusive on both ends." },
    { id: "gen-float-range", name: "Random Float in Range", cat: "Random", ignoresInput: true, params: [{ name: "min", label: "Min", type: "number", def: 0 }, { name: "max", label: "Max", type: "number", def: 1 }], run: (s, p) => CL.randomFloatInRange(p.min, p.max), about: "A random decimal number between min and max." },
...
<svg class="mascot-svg mood-${mood}" viewBox="0 0 132 124" width="111" height="104" fill="none">
  ...
  const DEMO_FIELD2 = { xor: "Key", vigenere: "Key", beaufort: "Key", caesar: "Shift", regex: "Pattern (regex)", railfence: "Rails", columnar: "Keyword", bitmask: "Mask (hex byte)", affine: "Key (a,b)", playfair: "Keyword", autokey: "Keyword", twoscomplement: "Bit width" };
  function buildDemo(view, demo) {
    ...
    if (DEMO_FIELD2[demo.type]) {
      ...
      in2.value = demo.type === "caesar" || demo.type === "railfence" ? "3" : demo.type === "columnar" ? "ZEBRAS" : demo.type === "regex" ? "\\d+" : demo.type === "bitmask" ? "0f" : demo.type === "affine" ? "5,8" : demo.type === "playfair" ? "PLAYFAIR" : demo.type === "autokey" ? "KEY" : demo.type === "twoscomplement" ? "8" : "key";
  ...
  async function demoCompute(t, v, k) {
    switch (t) {
      ...
      case "playfair": return CL.playfairEncode(v, k);
      case "autokey": return CL.autokeyEncode(v, k);
      case "twoscomplement": return CL.toTwosComplement(v, parseInt(k, 10) || 8);
